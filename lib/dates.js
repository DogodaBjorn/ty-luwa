...
/**
 * "zaterdag 11 juli t/m zaterdag 18 juli 2026". Met jaartal, want aanvragen
 * lopen tot twee jaar vooruit en dan is het jaar geen detail.
 */
function formatRangeLong(arrival, departure, lang, joiner = "t/m") {
  const sameYear = arrival.slice(0, 4) === departure.slice(0, 4);
  const from = sameYear
    ? fmt(arrival, lang, { weekday: "long", day: "numeric", month: "long" })
    : formatLong(arrival, lang);
  return `${from} ${joiner} ${formatLong(departure, lang)}`;
}

/** "14 juli" */
function formatDayMonth(s, lang) {
  return fmt(s, lang, { day: "numeric", month: "long" });
}

/** "vandaag", "over 12 dagen", "over 10 maanden" — hoe ver een datum weg is. */
function relativeAhead(from, to, lang = "nl") {
  const days = toDays(to) - toDays(from);
  if (days === 0) return lang === "nl" ? "vandaag" : "today";
  if (days < 0) return lang === "nl" ? "in het verleden" : "in the past";
  if (days === 1) return lang === "nl" ? "morgen" : "tomorrow";
  if (days < 31) return lang === "nl" ? `over ${days} dagen` : `in ${days} days`;
  const months = Math.round(days / 30.44);
  if (months < 24) {
    return lang === "nl"
      ? `over ${months} ${months === 1 ? "maand" : "maanden"}`
      : `in ${months} month${months === 1 ? "" : "s"}`;
  }
  const years = Math.round(days / 365.25);
  return lang === "nl" ? `over ${years} jaar` : `in ${years} years`;
}
