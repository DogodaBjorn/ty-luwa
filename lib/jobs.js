...
function createJobs({ store, db, mailer, holidaySource, config, log = console, now = () => new Date() }) {
  ...
  /**
   * Feestdagen en schoolvakanties bijwerken: dit jaar en volgend jaar, per
   * land. Eén keer per dag genoeg. Een land dat niet meewerkt laat zijn
   * vorige lijst gewoon staan.
   */
  async function runHolidaySync(force = false) {
    if (!holidaySource) return [];
    const day = now().toISOString().slice(0, 10);
    if (!force && store.getMeta("holidays_synced") === day) return [];
    const year = now().getUTCFullYear();
    const done = [];
    for (const country of COUNTRIES) {
      for (const y of [year, year + 1]) {
        try {
          const rows = await holidaySource.fetchYear(country, y);
          store.replaceHolidays(country, y, rows);
          const pub = rows.filter((r) => r.kind === "public").length;
          log.log(`feestdagen ${country} ${y}: ${pub} feestdagen, ${rows.length - pub} vakanties`);
          done.push(`${country}-${y}`);
        } catch (e) {
          log.error(`Feestdagen ${country} ${y} ophalen mislukt:`, e.message);
        }
      }
    }
    if (done.length) store.setMeta("holidays_synced", day);
    return done;
  }
  ...
  async function tick() {
    for (const [name, fn] of [["opruimen", runCleanup], ["siblu", runSibluSeed], ["feestdagen", runHolidaySync], ["snapshot", runSnapshot], ["back-upmail", runBackupMail]]) {
      try {
        await fn();
      } catch (e) {
  ...
  return { runCleanup, runSibluSeed, runHolidaySync, runSnapshot, runBackupMail, tick, start, backupDir };
}
