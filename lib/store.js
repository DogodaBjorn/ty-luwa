...
const PERIOD_KINDS = ["rented", "own", "option", "blocked"];
...
function createStore(db, opts = {}) {
  ...
  function importSnapshot(snap) {
    transaction(() => {
      db.exec("DELETE FROM periods; DELETE FROM requests;");
      const cols = (rows) => Object.keys(rows[0] || {});
      for (const table of ["requests", "periods"]) {
  ...
  return {
    ...
    setRequestStatus, setMailStatus, periodsOfRequest,
