...
function createJobs({ store, db, mailer, config, log = console, now = () => new Date() }) {
  ...
  async function tick() {
    for (const [name, fn] of [["opruimen", runCleanup], ["snapshot", runSnapshot], ["back-upmail", runBackupMail]]) {
      try {
        await fn();
      } catch (e) {
  ...
  function start(intervalMs = 60 * 60 * 1000) {
    // Niet meteen bij het opstarten: eerst de site bedienen.
  ...
  return { runCleanup, runSnapshot, runBackupMail, tick, start, backupDir };
}
